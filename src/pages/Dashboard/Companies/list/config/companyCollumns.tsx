...
const companyCollumns = ({
  ...
  const handleDeleteClick = (id: GridRowId) => async () => {
    props.setStatusButtons(true);
    await confirm({ description: `Deseja apagar a empresa ${id}`, title: 'Aviso' })
      .then(async () => {
        const newRows = props.rows?.filter(row => row.id !== id);
        const { response, error } = await useAxios({ method: 'delete', url: `company/${id}` });
        if (response) {
  ...
  return [
    { field: 'id', headerName: 'ID', flex: 1, hide: true },
    {
      ...
      flex: 1,
      editable: true,
      description: 'Nome visivel da sua empresa',
    ...
    },
    {
      ...
      flex: 1,
      editable: true,
      description: 'Uma descrição curta para sua empresa',
      renderCell: (params: GridRenderCellParams): JSX.Element => {
    ...
    },
    {
      ...
      flex: 1,
      editable: false,
      description: 'Usuario vinculado a esta empresa',
      renderCell: (params: GridRenderCellParams): JSX.Element => {
        return <TooltipCell params={params} collumns='user_id' />;
      },
    },
    {
      field: 'actions',
      ...
      sortable: false,
      flex: 1,
      description: 'Edite e exclua empresas',
...
export default companyCollumns;
